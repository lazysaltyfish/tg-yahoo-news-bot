...
def translate_text(text_to_translate: str) -> str | None:
    """
    Translates the given text to Chinese using the translation API (POST method).

    Args:
        text_to_translate: The text string to translate.

    Returns:
        The translated text string, or None if an error occurs.
    """
    ...
    if data is None:
        logger.error("Failed to fetch or parse translation.")
        return None

    # Basic validation (adapt based on actual API response structure)
    # Assuming the API returns a dict like {'translated_text': '...'}
    if not isinstance(data, dict) or 'translated_text' not in data:
        logger.error(f"Unexpected data format received from translation API. Expected dict with 'translated_text', got: {data}")
        return None
